...
#if defined(_TARGET_ARM_) || defined(_TARGET_ARM64_) || defined(_TARGET_X86_) || defined(_TARGET_MIPS64_)
...
UINT_PTR ExceptionTracker::CallCatchHandler(CONTEXT* pContextRecord, bool* pfAborting /*= NULL*/)
{
    ...
    if (!fIntercepted)
    {
        ...
        uResumePC = CallHandler(m_uCatchToCallPC, sfStackFp, &m_ClauseForCatch, pMD, Catch X86_ARG(pContextRecord) ARM_ARG(pContextRecord) ARM64_ARG(pContextRecord) MIPS64_ARG(pContextRecord) MIPS64_ARG(pMD->GetNativeCode()));
    }
    else
    {
                        ...
                        if (! IsStackOverflowException())
                        {
                            ...
                            EX_TRY
                            {
...
#endif // USE_FUNCLET_CALL_HELPER
                                {
                                    ...
                                    GCX_COOP();
                                    dwResult = CallHandler(dwFilterStartPC, sf, &EHClause, pMD, Filter X86_ARG(pContext) ARM_ARG(pContext) ARM64_ARG(pContext) MIPS64_ARG(pContext) MIPS64_ARG(pJitMan->JitTokenToStartAddress(MethToken)));
                                }
                            }
                            EX_CATCH
                            {
                    ...
                    if (fFoundHandler)
                    {
                        ...
                        else
                        {
                            ...
                            if (!IsStackOverflowException())
                            {
                                ...
                                ENDFORBIDGC();
                                dwStatus = CallHandler(dwHandlerStartPC, sf, &EHClause, pMD, FaultFinally X86_ARG(pcfThisFrame->GetRegisterSet()->pCurrentContext) ARM_ARG(pcfThisFrame->GetRegisterSet()->pCurrentContext) ARM64_ARG(pcfThisFrame->GetRegisterSet()->pCurrentContext)  MIPS64_ARG(pcfThisFrame->GetRegisterSet()->pCurrentContext) MIPS64_ARG(pJitMan->JitTokenToStartAddress(MethToken)));
...
EXTERN_C DWORD_PTR STDCALL CallEHFilterFunclet(Object *pThrowable, TADDR CallerSP, UINT_PTR pFuncletToInvoke, UINT_PTR *pFuncletCallerSP MIPS64_ARG(TADDR _Gp));
...
DWORD_PTR ExceptionTracker::CallHandler(
    ...
    MIPS64_ARG(PCONTEXT pContextRecord)
    MIPS64_ARG(UINT_PTR uGp)
    )
{
    STATIC_CONTRACT_THROWS;
    STATIC_CONTRACT_GC_TRIGGERS;
    ...
    else
    {
        ...
        dwResumePC = CallEHFilterFunclet(OBJECTREFToObject(throwable),
                                         ...
                                         CastHandlerFn(pfnHandler),
                                         pFuncletCallerSP MIPS64_ARG(uGp));
    }
...
#if defined(__linux__) && (defined(_TARGET_ARM_) || defined(_TARGET_ARM64_) || defined(_TARGET_MIPS64_))
...
#elif defined(_TARGET_MIPS64_)
    // But keep the architecture flag set (its part of CONTEXT_DEBUG_REGISTERS)
    pDispatcherContext->ContextRecord->ContextFlags |= CONTEXT_MIPS64;
#else // _TARGET_ARM64_
    // But keep the architecture flag set (its part of CONTEXT_DEBUG_REGISTERS)
