  ...
  Postconditions:
    ...
    - GenTree::GetRegNum() (and gtRegPair for ARM) is annotated with the register
      ...
      - However, if such a node is constrained to a set of registers, and its current location does not satisfy that
        requirement, LSRA must insert a GT_COPY node between the node and its parent.  The GetRegNum() on the GT_COPY node
    ...
    - LclVarDsc::GetRegNum() / GetOtherReg(): these are initialized to their
        ...
        for (lclNum = 0, varDsc = compiler->lvaTable; lclNum < compiler->lvaCount; lclNum++, varDsc++)
        {
            ...
            else
            {
                ...
                if (varDsc->lvIsParam)
                {
                    ...
                    {
                        // TODO-ARM-NYI: Map the hi/lo intervals back to GetRegNum() and GetOtherReg() (these should NYI
                        // before this)
                ...
                // If GetRegNum() is REG_STK, that means that either no register
                ...
                {
                    // For codegen purposes, we'll set GetRegNum() to whatever register
