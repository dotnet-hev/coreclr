...
{
    assert((regsMask & (RBM_CALLEE_SAVED | RBM_GP | RBM_FP | RBM_RA)) == regsMask); // Do not expect anything else.
...
void CodeGen::genSaveCalleeSavedRegistersHelp(regMaskTP regsToSaveMask, int lowestCalleeSavedOffset, int spDelta)
{
    ...
    assert(regsToSaveCount <= genCountBits(RBM_CALLEE_SAVED | RBM_GP));
...
void CodeGen::genRestoreCalleeSavedRegistersHelp(regMaskTP regsToRestoreMask, int lowestCalleeSavedOffset, int spDelta)
{
    ...
    assert(regsToRestoreCount <= genCountBits(RBM_CALLEE_SAVED | RBM_GP | RBM_FP | RBM_RA));
...
void CodeGen::genFuncletProlog(BasicBlock* block)
{
    ...
    if (compiler->lvaPSPSym != BAD_VAR_NUM)
    {
        if (isFilter)
        {
            ...
            getEmitter()->emitIns_R_R_I(INS_ori, EA_PTRSIZE, REG_GP, REG_A4, 0);
        }
...
void CodeGen::instGen_Set_Reg_To_Imm(emitAttr size, regNumber reg, ssize_t imm, insFlags flags)
{
    ...
    {
        emit->emitIns_I_mips(size, reg, imm);
    }
    ...
    switch (tree->gtOper)
    {
        ...
        case GT_CNS_DBL:
        {
            ...
            else
            {
                ...
                assert(targetReg > REG_RA);

                instruction ins = size == EA_4BYTE ? INS_lwc1 : INS_ldc1;
        ...
        else // the divisor is not the constant zero
        {
            ...
            if (tree->gtOper == GT_DIV || tree->gtOper == GT_MOD)
            {
                ...
                else // insert check for divison by zero
                {
                    ...
                    BasicBlock* tmpLabel = genCreateTempLabel();
                    getEmitter()->emitIns_J_cond_mips(INS_bne, tmpLabel, divisorReg, REG_R0);

                    genJumpToThrowHlpBlk(EJ_jmp, SCK_DIV_BY_ZERO);//EJ_jmp is 2/5-ins
                    genDefineTempLabel(tmpLabel);// optimize ???
                }
                ...
                if (checkDividend)
                {
                    ...
                    inst_JMP(EJ_jmp, sdivLabel);//no branch-delay. 2-ins.
                    ...
                    getEmitter()->emitIns_J_cond_mips(INS_beq, sdivLabel, dividendReg, REG_R0);

                    getEmitter()->emitIns_R_R_R(size == EA_4BYTE ? INS_addu : INS_daddu, size, REG_AT, dividendReg, dividendReg);

                    getEmitter()->emitIns_J_cond_mips(INS_bne, sdivLabel, REG_AT, REG_R0);

                    genJumpToThrowHlpBlk(EJ_jmp, SCK_ARITH_EXCPN); //EJ_jmp is 2/5-ins.

                    genDefineTempLabel(sdivLabel);// optimize ???
                }
...
void CodeGen::genCkfinite(GenTree* treeNode)
{
    ...
    BasicBlock* tmpLabel = genCreateTempLabel();
    getEmitter()->emitIns_J_cond_mips(INS_bne, tmpLabel, intReg, REG_R0);

    // If exponent is all 1's, throw ArithmeticException
    genJumpToThrowHlpBlk(EJ_jmp, SCK_ARITH_EXCPN); //EJ_jmp is 2/5-ins.

    genDefineTempLabel(tmpLabel);
    ...
    else
    {
        ...
        if (op2->isContainedIntOrIImmed())
        {
            ...
            else if (tree->OperIs(GT_LT))
            {
                ...
                {
                    emit->emitIns_I_mips(EA_PTRSIZE, REG_AT, imm);
...
void CodeGen::genCodeForJumpTrue(GenTreeOp* jtrue)
{
    ...
    BasicBlock* tmpLabel = genCreateTempLabel();
    getEmitter()->emitIns_J_cond_mips(INS_beq, tmpLabel, SaveCcResultReg, REG_R0);
...
void CodeGen::genCodeForJumpCompare(GenTreeOp* tree)
{
    ...
    {// optimize ???
        ...
        BasicBlock* tmpLabel = genCreateTempLabel();
        getEmitter()->emitIns_J_cond_mips(ins, tmpLabel, reg, REG_R0);
...
void CodeGen::genRangeCheck(GenTree* oper)
{
    ...
    if (arrIndex->isContainedIntOrIImmed())
    {
        ...
        {
            emit->emitIns_R_R_I(INS_daddiu, EA_PTRSIZE, REG_AT, REG_R0, -1);
            emit->emitIns_R_R_I(INS_dsrl, EA_PTRSIZE, REG_AT, REG_AT, 1);
        }
        else
        {
            emit->emitIns_I_mips(EA_PTRSIZE, REG_AT, imm);
        ...
        if (imm == INT64_MAX)
        {
            ...
            emit->emitIns_R_R_I(INS_daddiu, EA_PTRSIZE, REG_AT, REG_R0, imm2);
            imm2 = 1;
            emit->emitIns_R_R_I(INS_dsrl, EA_PTRSIZE, REG_AT, REG_AT, imm2);
        }
        else
        {
    ...
    getEmitter()->emitIns_J_cond_mips(INS_bne, tmpLabel, REG_AT, REG_R0);
    ...
    genJumpToThrowHlpBlk(jmpKind, bndsChk->gtThrowKind, bndsChk->gtIndRngFailBB); //EJ_jmp is 2/5-ins.
    genDefineTempLabel(tmpLabel);
}
...
void CodeGen::genCodeForArrIndex(GenTreeArrIndex* arrIndex)
{
    ...
    genJumpToThrowHlpBlk(EJ_jmp, SCK_RNGCHK_FAIL); //EJ_jmp is 2/5-ins.
...
void CodeGen::genCodeForIndexAddr(GenTreeIndexAddr* node)
{
    ...
    if ((node->gtFlags & GTF_INX_RNGCHK) != 0)
    {
        ...
        genJumpToThrowHlpBlk(EJ_jmp, SCK_RNGCHK_FAIL, node->gtIndRngFailBB); //EJ_jmp is 2/5-ins.
        genDefineTempLabel(tmpLabel);
    }
...
void CodeGen::genIntCastOverflowCheck(GenTreeCast* cast, const GenIntCastDesc& desc, regNumber reg)
{
    switch (desc.CheckKind())
    {
        case GenIntCastDesc::CHECK_POSITIVE:
        {
            ...
            ssize_t imm = compiler->fgUseThrowHelperBlocks() ? JMP_PLACEHOLD_SIZE_MIPS : (6 << 2);//size of Jump-placeholders in bytes liking within emitter::emitIns_J().
            getEmitter()->emitIns_R_R_I(INS_beq, EA_PTRSIZE, REG_AT, REG_R0, imm);
            getEmitter()->emitIns(INS_nop);

            genJumpToThrowHlpBlk(EJ_jmp, SCK_OVERFLOW);//EJ_jmp is 2/5-ins
        }
        break;
        ...
        case GenIntCastDesc::CHECK_UINT_RANGE:
        {
            ...
            imm = compiler->fgUseThrowHelperBlocks() ? JMP_PLACEHOLD_SIZE_MIPS : (6 << 2);
            getEmitter()->emitIns_R_R_I(INS_beq, EA_PTRSIZE, REG_AT, REG_R0, imm);
            getEmitter()->emitIns(INS_nop);
        ...
        case GenIntCastDesc::CHECK_INT_RANGE:
        {
            ...
            ssize_t imm = compiler->fgUseThrowHelperBlocks() ? JMP_PLACEHOLD_SIZE_MIPS : (6 << 2);
            getEmitter()->emitIns_R_R_I(INS_beq, EA_PTRSIZE, REG_AT, REG_R0, imm);
            getEmitter()->emitIns(INS_nop);
        ...
        default:
        {
            ...
            else
            {
                ...
                BasicBlock* tmpLabel = genCreateTempLabel();
                getEmitter()->emitIns_J_cond_mips(INS_beq, tmpLabel, REG_AT, REG_R0);
...
void CodeGen::genScaledAdd(emitAttr attr, regNumber targetReg, regNumber baseReg, regNumber indexReg, int scale)
{
    ...
    else
    {
        ...
        emit->emitIns_R_R_I(INS_dsll, attr, REG_AT, indexReg, scale);
